...
function HomeScreen({ navigation, combos, onDelete }) {
...
function AddComboScreen({ navigation, route, onAdd, onEdit }) {
...
export default function App() {
  ...
  const handleAdd = (newCombo) => {
    setCombos([...combos, { id: Date.now().toString(), ...newCombo }]);
  };

  const handleEdit = (updatedCombo) => {
  ...
  return (
    <NavigationContainer>
      ...
      >
        <Stack.Screen name="Home" options={{ title: 'My Combos' }}>
          {(props) => <HomeScreen {...props} combos={combos} onDelete={handleDelete} />}
        </Stack.Screen>
